...
const startGame=()=>{
    ...
    socket.emit("start_game",{userId:user._id, roomId:room._id,gameData: { ...playerCards, drawpile: pile, discardpile: cards }})
}
...
const leaveRoom=()=>{
  ...
  useEffect(() => {
    setUser({ ...user, cards: playerCards });
}, [playerCards]);
    ...
    room && <div>
        ...
        </div>}
        {
            user?.cards?.map((card,i)=>{
                return <div key={card.number + i}>{card.number} {card.color}</div>
            })
        }
